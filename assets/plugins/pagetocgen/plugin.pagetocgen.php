...
if ($option1[1] !='') {
    $toc_list_type = $option1[1];
} else {
    $toc_list_type = $default_toc_list_type;
}
...
if ($option5[1] !='') {
    $toc_heading_tag = $option5[1];
} else {
    $toc_heading_tag = $default_toc_heading_tag;
}
...
if ($option6[1] !='') {
    $toc_parent = $option6[1];
} else {
    $toc_parent = $default_toc_parent;
}
...
if ($option7[1] !='') {
    $toc_parent_id = $option7[1];
} else {
    $toc_parent_id = $default_toc_parent_id;
}
...
$source = str_replace(
    $open_param.$start_config_marker.$close_param
    , $display_content
    , $source
);
$source = str_replace(
    array($open_param . $toc_list_type_marker . $toc_list_type . $close_param,
        $open_param . $start_level_marker . $start_level . $close_param,
        $open_param . $end_level_marker . $end_level . $close_param,
        $open_param . $toc_heading_text_marker . $toc_heading_text . $close_param,
        $open_param . $toc_heading_marker . $toc_heading_tag . $close_param,
        $open_param . $toc_parent_marker . $toc_parent . $close_param,
        $open_param . $toc_parent_id_marker . $toc_parent_id . $close_param,
        $open_param . $start_marker . $close_param,
        $open_param . $end_config_marker . $close_param,
        $open_param . $output_marker . $close_param,
        $open_param . $end_marker . $close_param)
    , ''
    , $source
);
...
{
    $return_str = "";
    for($i=1, $iMax = strlen($val); $i <= $iMax; $i++)
    {
